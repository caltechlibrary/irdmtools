...
# Comprehensive Summary of Invenio RDM Search JSON API Endpoints

> - Invenio RDM search API leverages OpenSearch for powerful search capabilities.
> - Key search endpoints include `/api/records`, `/api/drafts`, `/api/files`, and `/api/requests`.
> - Search endpoints support pagination, sorting, filtering, and pretty-printing of JSON responses.
> - Authentication via Bearer tokens is required for most search endpoints.
> - The API follows RESTful conventions with JSON payloads aligned with DataCite Schema v4.x.

---

## Introduction

The Invenio RDM (Research Data Management) platform provides a robust search JSON API for managing research data records, drafts, files, and user requests. This report synthesizes the official documentation into a structured, comprehensive Markdown table summarizing all search API endpoints, their methods, descriptions, example requests, responses, and notes. The table is designed to be a ready-reference guide for developers and researchers interacting with Invenio RDM programmatically, with a focus on search functionality and its alignment with OpenSearch.

---

## Authentication and Authorization

The Invenio RDM search API uses Bearer tokens for authentication. These tokens are generated in the user account settings and must be included in requests either via the `Authorization` HTTP header or as an `access_token` query parameter. Authentication is mandatory for most search endpoints, especially those involving drafts, records, and requests.
...
## Search API Endpoints Summary Table
