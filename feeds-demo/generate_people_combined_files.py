...
def render_combined_files(repo, d_name, people_id, person):
    ...
    # Write out RSS file.
    f_name = os.path.join(d_name, o_name + '.rss')
    label = mk_label(o_name)
...
def render_editor_files(d_name, obj, people_id, person):
    ...
    if repo_resources is not None:
        ...
        if len(objects) > 0:
            ...
            write_bibtex_file(f_name, objects)
            # Write out RSS file
            f_name = os.path.join(d_name, 'editor.rss')
            write_rss_file(f_name, 'Editor Feed', objects)
...
def render_advisor_files(d_name, obj, people_id, person):
    ...
    if repo_resources is not None:
        ...
        if len(objects) > 0:
            ...
            write_bibtex_file(f_name, objects)
            # Write out RSS file
            f_name = os.path.join(d_name, 'advisor.rss')
            write_rss_file(f_name, 'Advisor Feed', objects)
...
def render_committee_files(d_name, obj, people_id, person):
    ...
    if repo_resources is not None:
        ...
        if len(objects) > 0:
            ...
            write_bibtex_file(f_name, objects)
            # Write out RSS file
            f_name = os.path.join(d_name, 'committee.rss')
            write_rss_file(f_name, 'Committee Feed', objects)
...
def render_peoples(people_list, people_id = None):
    ...
    if os.path.exists(f_name):
        ...
        print(f'{f_name} not found, aborting')
        sys.exit(10)
