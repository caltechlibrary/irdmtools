...
var (
	TestDefaultOptions = []byte(`# This YAML file controls the mappings of 
...
issn_journals:
  ...
  0028-0836: Nature
  0035-2966: Monthly Notices of the Royal Astronomical Society
  ...
  1365-2966: Monthly Notices of the Royal Astronomical Society
...
func TestIssue81(t *testing.T) {
	...
	//log.Printf(`DEBUG obj -> %+v`, workObj.Message)
	...
	if record.CustomFields != nil {
		if journalJournal, ok := record.CustomFields["journal:journal"].(map[string]interface{}); ok {
			if title, ok := journalJournal["title"].(string); ok {
				if strings.Contains(title, "Astronomial") {
					t.Errorf("`.custom_fields[\"journal:journal\"].title` contains 'Anstronomial' instead of 'Astronomical', %q", title)
				}
				if strings.Contains(title, " Notice ") {
					t.Errorf("`.custom_fields[\"journal:journal\"].title` contains ' Notice ' instead of 'Notices', %q", title)
				}
			} else {
				t.Errorf("failed to fund `.custom_fields[\"journal:journal\"].title` in record")
			}
		} else {
			t.Errorf("failed to fund `.custom_fields[\"journal:journal\"]` in record")
		}
	} else {
